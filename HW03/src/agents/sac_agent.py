...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            self.log_alpha = nn.Parameter(torch.tensor(np.log(self.temperature), dtype=torch.float32))
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values = reward + (1 - done.float()) * self.discount * next_qs 
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # loss = -q_min.mean()
        loss = (self.get_temperature() * log_prob - q_min).mean()
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        #     loss = loss - self.get_temperature() * entropy.mean()
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        """
        Update temperature parameter using dual gradient descent.
