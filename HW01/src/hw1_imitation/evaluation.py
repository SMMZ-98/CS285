...
from src.hw1_imitation.data import Normalizer
from src.hw1_imitation.model import BasePolicy
import copy
...
class Logger:
    ...
    def __init__(self, path: Path):
        ...
        self.path = path
        self.csv_path = path / "log.csv"
        self.header: set[str] = set()  # Use a set to track all seen columns
        self.rows: list[dict[str, Any]] = []  # Store all rows

    def log(self, row: dict[str, Any], step: int) -> None:
        row["step"] = step
        
        # Filter out non-CSV types
        filtered_row = {
        ...
        new_columns = set(filtered_row.keys()) - self.header
        
        if new_columns:
            # Update header with new columns
            self.header.update(new_columns)
            
            # Rewrite entire CSV with updated header
            sorted_header = sorted(self.header)  # Sort for consistent column order
            
            with self.csv_path.open("w") as f:
                f.write(",".join(sorted_header) + "\n")
                
                # Write all previous rows
                for old_row in self.rows:
                    f.write(
                        ",".join([str(old_row.get(k, "")) for k in sorted_header]) + "\n"
                    )
                
                # Write current row
                f.write(
                    ",".join([str(filtered_row.get(k, "")) for k in sorted_header]) + "\n"
                )
        else:
            # Just append current row
            sorted_header = sorted(self.header)
            with self.csv_path.open("a") as f:
                f.write(
                    ",".join([str(filtered_row.get(k, "")) for k in sorted_header]) + "\n"
                )
        
        # Store row for potential future rewrites
        self.rows.append(copy.deepcopy(filtered_row))
